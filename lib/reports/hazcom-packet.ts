...
<head>
  <meta charset="utf-8">
  <title>${escapeHtml(terms.pdfDocumentTitle)} - ${escapeHtml(org.name)}</title>
  <style>
...
<body>
  <h1>${escapeHtml(terms.pdfDocumentTitle)}</h1>
  ...
  <div class="disclaimer">
    ${escapeHtml(terms.disclaimer)} This packet reflects data in SafeCellar at generation time; verify SDS files match products in use before an ${escapeHtml(terms.inspectionContext)}.
  </div>

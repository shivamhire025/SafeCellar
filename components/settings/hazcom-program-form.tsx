...
export function HazcomProgramForm({ org }: { org: Organization }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast({ title: "HazCom program saved", variant: "success" });
      router.refresh();
    } catch {
  ...
  return (
    <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 max-w-2xl">
      ...
      <p className="text-sm text-neutral-500 mb-6">
        These sections appear in your OSHA inspection packet export. Leave blank to use standard templates.
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Button variant="secondary" asChild>
          <a href="/api/reports/hazcom?format=pdf">Download PDF</a>
        </Button>
        <Button variant="secondary" asChild>
          <a href="/api/reports/inspection-packet">Download inspection packet (ZIP)</a>
        </Button>
      </div>

      <p className="text-xs text-neutral-500 mt-4">
        SafeCellar assists with documentation. Your facility remains responsible for OSHA compliance.
      </p>
    </div>
  );
}
